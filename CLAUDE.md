...
6. **Telegram Bot** (`telegram_bot.py`, `telegram_notifications.py`, `telegram_calendar.py`)
...
7. **Очередь уведомлений** (`telegram_queue.py`, `telegram_notifications_queue.py`)
...
8. **Кэширование** (`cache_manager.py`)
   - Redis кэш для ускорения API запросов в 5.77 раз
   - Автоматическая инвалидация при добавлении новых отзывов
...
- **Общее количество отзывов**: 18,760+
- **Отзывов с фотографиями**: 2,642+ (14.1%)
- **Активных филиалов**: 23 из 23
- **Автоматический парсинг**: ✅ Настроен (каждые 5 минут)
- **Telegram Bot**: ✅ Активен с главным меню и календарем
...
- **Кэширование**: ✅ Redis с автоинвалидацией (ускорение в 5.77x)
- **Безопасность**: ✅ Токены в переменных окружения, ограниченный CORS, security заголовки
- **Отказоустойчивость**: ✅ Graceful fallback, автовосстановление
- **Тестирование**: ✅ 11 тестовых файлов, комплексное покрытие 96%
...
├── requirements.txt             # Python зависимости (+ python-telegram-bot)
...
---
Последнее обновление: 2025-07-10 14:35  
Версия: 2.5.0 (с безопасностью, очередями, кэшированием и комплексным тестированием)